...
library IbcApplicationsInterchain_accountsControllerV1MsgSendTx {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3) {
        pointer += _read_packet_data(pointer, bs, r);
      } else
      if (fieldId == 4) {
   ...
   * @return The number of bytes decoded
   */
  function _read_packet_data(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (IbcApplicationsInterchain_accountsV1InterchainAccountPacketData.Data memory x, uint256 sz) = _decode_IbcApplicationsInterchain_accountsV1InterchainAccountPacketData(p, bs);
    r.packet_data = x;
    return sz;
  }
   ...
   * @return The number of bytes decoded
   */
  function _read_relative_timeout(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
   ...
   */
  function _decode_IbcApplicationsInterchain_accountsV1InterchainAccountPacketData(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcApplicationsInterchain_accountsV1InterchainAccountPacketData.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcApplicationsInterchain_accountsV1InterchainAccountPacketData.Data memory r, ) = IbcApplicationsInterchain_accountsV1InterchainAccountPacketData._decode(pointer, bs, sz);
    ...
    returns (uint)
  {
    ...
    pointer += IbcApplicationsInterchain_accountsV1InterchainAccountPacketData._encode_nested(r.packet_data, pointer, bs);
    ...
    pointer += ProtoBufRuntime._encode_uint64(r.relative_timeout, pointer, bs);
    }
    return pointer - offset;
  }
  // nested encoder
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.owner).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.connection_id).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcApplicationsInterchain_accountsV1InterchainAccountPacketData._estimate(r.packet_data));
    e += 1 + ProtoBufRuntime._sz_uint64(r.relative_timeout);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    output.owner = input.owner;
    output.connection_id = input.connection_id;
    IbcApplicationsInterchain_accountsV1InterchainAccountPacketData.store(input.packet_data, output.packet_data);
...
library IbcApplicationsInterchain_accountsControllerV1MsgSendTxResponse {
