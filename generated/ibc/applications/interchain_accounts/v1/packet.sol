...
library IbcApplicationsInterchain_accountsV1InterchainAccountPacketData {
  ...
  //struct definition
  struct Data {
    IBC/APPLICATIONS/INTERCHAIN_ACCOUNTS/V1/PACKET_PROTO_GLOBAL_ENUMS.Type f_type;
    bytes data;
    string memo;
  }
  ...
  function _read_f_type(
    ...
    IBC/APPLICATIONS/INTERCHAIN_ACCOUNTS/V1/PACKET_PROTO_GLOBAL_ENUMS.Type x = IBC/APPLICATIONS/INTERCHAIN_ACCOUNTS/V1/PACKET_PROTO_GLOBAL_ENUMS.decode_Type(tmp);
    r.f_type = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    int32 _enum_f_type = IBC/APPLICATIONS/INTERCHAIN_ACCOUNTS/V1/PACKET_PROTO_GLOBAL_ENUMS.encode_Type(r.f_type);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_enum(IBC/APPLICATIONS/INTERCHAIN_ACCOUNTS/V1/PACKET_PROTO_GLOBAL_ENUMS.encode_Type(r.f_type));
...
library IbcApplicationsInterchain_accountsV1CosmosTx {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_messages(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_messages(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.messages.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += GoogleProtobufAny._encode_nested(r.messages[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.messages.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(GoogleProtobufAny._estimate(r.messages[i]));
    }
    return e;
  }
  // empty checker
   ...
   */
  function addMessages(Data memory self, GoogleProtobufAny.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    GoogleProtobufAny.Data[] memory tmp = new GoogleProtobufAny.Data[](self.messages.length + 1);
...
library IBC/APPLICATIONS/INTERCHAIN_ACCOUNTS/V1/PACKET_PROTO_GLOBAL_ENUMS {
