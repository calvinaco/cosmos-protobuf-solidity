...
library IbcApplicationsTransferV1MsgTransfer {
  ...
  struct Data {
    ...
    CosmosBaseV1beta1Coin.Data token;
    string sender;
    string receiver;
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3) {
        pointer += _read_token(pointer, bs, r);
      } else
      if (fieldId == 4) {
    ...
    returns (uint)
  {
    ...
    pointer += CosmosBaseV1beta1Coin._encode_nested(r.token, pointer, bs);
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosBaseV1beta1Coin._estimate(r.token));
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    CosmosBaseV1beta1Coin.store(input.token, output.token);
