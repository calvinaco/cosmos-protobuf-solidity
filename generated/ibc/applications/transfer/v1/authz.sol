...
library IbcApplicationsTransferV1Allocation {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3) {
        pointer += _read_unpacked_repeated_spend_limit(pointer, bs, nil(), counters);
      } else
      if (fieldId == 4) {
        pointer += _read_unpacked_repeated_allow_list(pointer, bs, nil(), counters);
      } else
      {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 3) {
        pointer += _read_unpacked_repeated_spend_limit(pointer, bs, r, counters);
      } else
      if (fieldId == 4) {
        pointer += _read_unpacked_repeated_allow_list(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.spend_limit.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += CosmosBaseV1beta1Coin._encode_nested(r.spend_limit[i], pointer, bs);
    }
    }
    ...
    for(i = 0; i < r.allow_list.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += ProtoBufRuntime._encode_string(r.allow_list[i], pointer, bs);
    }
    }
  ...
  function _estimate(
    ...
    for(i = 0; i < r.spend_limit.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosBaseV1beta1Coin._estimate(r.spend_limit[i]));
    }
    for(i = 0; i < r.allow_list.length; i++) {
   ...
   */
  function addSpendLimit(Data memory self, CosmosBaseV1beta1Coin.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosBaseV1beta1Coin.Data[] memory tmp = new CosmosBaseV1beta1Coin.Data[](self.spend_limit.length + 1);
...
library IbcApplicationsTransferV1TransferAuthorization {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_allocations(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    if (counters[1] > 0) {
      ...
      r.allocations = new IbcApplicationsTransferV1Allocation.Data[](counters[1]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_allocations(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
  ...
  function _read_unpacked_repeated_allocations(
     ...
     * if `r` is NULL, then only counting the number of fields.
     */
    (IbcApplicationsTransferV1Allocation.Data memory x, uint256 sz) = _decode_IbcApplicationsTransferV1Allocation(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
   ...
   * @return The number of bytes used to decode
   */
  function _decode_IbcApplicationsTransferV1Allocation(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcApplicationsTransferV1Allocation.Data memory, uint)
  {
    ...
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcApplicationsTransferV1Allocation.Data memory r, ) = IbcApplicationsTransferV1Allocation._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.allocations.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += IbcApplicationsTransferV1Allocation._encode_nested(r.allocations[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.allocations.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(IbcApplicationsTransferV1Allocation._estimate(r.allocations[i]));
    }
    return e;
  }
  // empty checker
   ...
   * @param value The value to add
   */
  function addAllocations(Data memory self, IbcApplicationsTransferV1Allocation.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    IbcApplicationsTransferV1Allocation.Data[] memory tmp = new IbcApplicationsTransferV1Allocation.Data[](self.allocations.length + 1);
