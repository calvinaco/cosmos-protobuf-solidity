...
library IbcCoreClientV1MsgCreateClientResponse {
...
library IbcCoreClientV1MsgUpdateClientResponse {
...
library IbcCoreClientV1MsgUpgradeClient {
  ...
  struct Data {
    ...
    GoogleProtobufAny.Data client_state;
    GoogleProtobufAny.Data consensus_state;
    bytes proof_upgrade_client;
    bytes proof_upgrade_consensus_state;
    string signer;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 4) {
        pointer += _read_proof_upgrade_client(pointer, bs, r);
      } else
      if (fieldId == 5) {
        pointer += _read_proof_upgrade_consensus_state(pointer, bs, r);
      } else
      if (fieldId == 6) {
        pointer += _read_signer(pointer, bs, r);
      } else
   ...
   * @return The number of bytes decoded
   */
  function _read_proof_upgrade_client(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (bytes memory x, uint256 sz) = ProtoBufRuntime._decode_bytes(p, bs);
    r.proof_upgrade_client = x;
    return sz;
  }
   ...
   * @return The number of bytes decoded
   */
  function _read_proof_upgrade_consensus_state(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (bytes memory x, uint256 sz) = ProtoBufRuntime._decode_bytes(p, bs);
    r.proof_upgrade_consensus_state = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    if (r.proof_upgrade_client.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      4,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.proof_upgrade_client, pointer, bs);
    }
    if (r.proof_upgrade_consensus_state.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      5,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.proof_upgrade_consensus_state, pointer, bs);
    }
    if (bytes(r.signer).length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      6,
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(GoogleProtobufAny._estimate(r.client_state));
    e += 1 + ProtoBufRuntime._sz_lendelim(GoogleProtobufAny._estimate(r.consensus_state));
    e += 1 + ProtoBufRuntime._sz_lendelim(r.proof_upgrade_client.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(r.proof_upgrade_consensus_state.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.signer).length);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    GoogleProtobufAny.store(input.client_state, output.client_state);
    GoogleProtobufAny.store(input.consensus_state, output.consensus_state);
    output.proof_upgrade_client = input.proof_upgrade_client;
    output.proof_upgrade_consensus_state = input.proof_upgrade_consensus_state;
    output.signer = input.signer;
...
library IbcCoreClientV1MsgUpgradeClientResponse {
...
library IbcCoreClientV1MsgSubmitMisbehaviourResponse {
