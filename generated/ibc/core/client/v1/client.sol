...
library IbcCoreClientV1ClientUpdateProposal {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3) {
        pointer += _read_subject_client_id(pointer, bs, r);
      } else
      if (fieldId == 4) {
        pointer += _read_substitute_client_id(pointer, bs, r);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_subject_client_id(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (string memory x, uint256 sz) = ProtoBufRuntime._decode_string(p, bs);
    r.subject_client_id = x;
    return sz;
  }
   ...
   * @return The number of bytes decoded
   */
  function _read_substitute_client_id(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (string memory x, uint256 sz) = ProtoBufRuntime._decode_string(p, bs);
    r.substitute_client_id = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    if (bytes(r.subject_client_id).length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      3,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_string(r.subject_client_id, pointer, bs);
    }
    if (bytes(r.substitute_client_id).length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      4,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_string(r.substitute_client_id, pointer, bs);
    }
    return pointer - offset;
  }
  // nested encoder
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.subject_client_id).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.substitute_client_id).length);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.subject_client_id = input.subject_client_id;
    output.substitute_client_id = input.substitute_client_id;
...
library IbcCoreClientV1UpgradeProposal {
  ...
  struct Data {
    string title;
    string description;
    CosmosUpgradeV1beta1Plan.Data plan;
  ...
  function _read_plan(
    ...
    (CosmosUpgradeV1beta1Plan.Data memory x, uint256 sz) = _decode_CosmosUpgradeV1beta1Plan(p, bs);
    r.plan = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosUpgradeV1beta1Plan(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosUpgradeV1beta1Plan.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosUpgradeV1beta1Plan.Data memory r, ) = CosmosUpgradeV1beta1Plan._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.title).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.description).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosUpgradeV1beta1Plan._estimate(r.plan));
  ...
  function store(Data memory input, Data storage output) internal {
    output.title = input.title;
    output.description = input.description;
    CosmosUpgradeV1beta1Plan.store(input.plan, output.plan);
...
library IbcCoreClientV1Params {


  //struct definition
  struct Data {
    string[] allowed_clients;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_allowed_clients(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    pointer = offset;
    if (counters[1] > 0) {
      require(r.allowed_clients.length == 0);
      r.allowed_clients = new string[](counters[1]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_allowed_clients(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_unpacked_repeated_allowed_clients(
    uint256 p,
    bytes memory bs,
    Data memory r,
    uint[2] memory counters
    ...
    if (isNil(r)) {
      ...
      r.allowed_clients[r.allowed_clients.length - counters[1]] = x;
      counters[1] -= 1;
    }
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    uint256 pointer = p;
    uint256 i;
    if (r.allowed_clients.length != 0) {
    for(i = 0; i < r.allowed_clients.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        1,
        ProtoBufRuntime.WireType.LengthDelim,
        pointer,
        bs)
      ;
      pointer += ProtoBufRuntime._encode_string(r.allowed_clients[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.allowed_clients.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.allowed_clients[i]).length);
    }
    return e;
  }
  // empty checker
   ...
   */
  function store(Data memory input, Data storage output) internal {
    output.allowed_clients = input.allowed_clients;

  }


  //array helpers for AllowedClients
  /**
   * @dev Add value to an array
   * @param self The in-memory struct
   * @param value The value to add
   */
  function addAllowedClients(Data memory self, string memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
