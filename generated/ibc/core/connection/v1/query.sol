...
library IbcCoreConnectionV1QueryConnectionRequest {
...
library IbcCoreConnectionV1QueryConnectionsRequest {
...
library IbcCoreConnectionV1QueryConnectionsResponse {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_connections(pointer, bs, nil(), counters);
      } else
      if (fieldId == 2) {
        pointer += _read_pagination(pointer, bs, r);
      } else
    ...
    if (counters[1] > 0) {
      ...
      r.connections = new IbcCoreConnectionV1IdentifiedConnection.Data[](counters[1]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_connections(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
  ...
  function _read_unpacked_repeated_connections(
     ...
     */
    (IbcCoreConnectionV1IdentifiedConnection.Data memory x, uint256 sz) = _decode_IbcCoreConnectionV1IdentifiedConnection(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
   ...
   * @return The number of bytes used to decode
   */
  function _decode_IbcCoreConnectionV1IdentifiedConnection(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcCoreConnectionV1IdentifiedConnection.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcCoreConnectionV1IdentifiedConnection.Data memory r, ) = IbcCoreConnectionV1IdentifiedConnection._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.connections.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += IbcCoreConnectionV1IdentifiedConnection._encode_nested(r.connections[i], pointer, bs);
    }
    }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.connections.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(IbcCoreConnectionV1IdentifiedConnection._estimate(r.connections[i]));
    }
   ...
   */
  function addConnections(Data memory self, IbcCoreConnectionV1IdentifiedConnection.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    IbcCoreConnectionV1IdentifiedConnection.Data[] memory tmp = new IbcCoreConnectionV1IdentifiedConnection.Data[](self.connections.length + 1);
...
library IbcCoreConnectionV1QueryClientConnectionsRequest {
...
library IbcCoreConnectionV1QueryClientConnectionsResponse {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_connection_paths(pointer, bs, nil(), counters);
      } else
      if (fieldId == 2) {
        pointer += _read_proof(pointer, bs, r);
      } else
    ...
    pointer = offset;
    if (counters[1] > 0) {
      require(r.connection_paths.length == 0);
      r.connection_paths = new string[](counters[1]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_connection_paths(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_unpacked_repeated_connection_paths(
    uint256 p,
    bytes memory bs,
    Data memory r,
    uint[4] memory counters
    ...
    if (isNil(r)) {
      ...
      r.connection_paths[r.connection_paths.length - counters[1]] = x;
      counters[1] -= 1;
    }
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    uint256 pointer = p;
    uint256 i;
    if (r.connection_paths.length != 0) {
    for(i = 0; i < r.connection_paths.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        1,
        ProtoBufRuntime.WireType.LengthDelim,
        pointer,
        bs)
      ;
      pointer += ProtoBufRuntime._encode_string(r.connection_paths[i], pointer, bs);
    }
    }
    if (r.proof.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.connection_paths.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.connection_paths[i]).length);
    }
    e += 1 + ProtoBufRuntime._sz_lendelim(r.proof.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcCoreClientV1Height._estimate(r.proof_height));
    return e;
   ...
   */
  function store(Data memory input, Data storage output) internal {
    output.connection_paths = input.connection_paths;
    output.proof = input.proof;
    IbcCoreClientV1Height.store(input.proof_height, output.proof_height);
  ...
  //array helpers for ConnectionPaths
  /**
   * @dev Add value to an array
   * @param self The in-memory struct
   * @param value The value to add
   */
  function addConnectionPaths(Data memory self, string memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
...
library IbcCoreConnectionV1QueryConnectionClientStateResponse {
...
library IbcCoreConnectionV1QueryConnectionConsensusStateRequest {
...
library IbcCoreConnectionV1QueryConnectionConsensusStateResponse {
...
library IbcCoreConnectionV1QueryConnectionParamsRequest {
...
library IbcCoreConnectionV1QueryConnectionParamsResponse {
  ...
  function _read_params(
    ...
    (IbcCoreConnectionV1Params.Data memory x, uint256 sz) = _decode_IbcCoreConnectionV1Params(p, bs);
    r.params = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_IbcCoreConnectionV1Params(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcCoreConnectionV1Params.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcCoreConnectionV1Params.Data memory r, ) = IbcCoreConnectionV1Params._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += IbcCoreConnectionV1Params._encode_nested(r.params, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcCoreConnectionV1Params._estimate(r.params));
    return e;
  }
  // empty checker
