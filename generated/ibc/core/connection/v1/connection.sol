...
library IbcCoreConnectionV1ConnectionEnd {
  ...
  struct Data {
    ...
    IBC/CORE/CONNECTION/V1/CONNECTION_PROTO_GLOBAL_ENUMS.State state;
  ...
  function _read_state(
    ...
    IBC/CORE/CONNECTION/V1/CONNECTION_PROTO_GLOBAL_ENUMS.State x = IBC/CORE/CONNECTION/V1/CONNECTION_PROTO_GLOBAL_ENUMS.decode_State(tmp);
    r.state = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.versions.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += IbcCoreConnectionV1Version._encode_nested(r.versions[i], pointer, bs);
    }
    }
    if (uint(r.state) != 0) {
    pointer += ProtoBufRuntime._encode_key(
    ...
    int32 _enum_state = IBC/CORE/CONNECTION/V1/CONNECTION_PROTO_GLOBAL_ENUMS.encode_State(r.state);
  ...
  function _estimate(
    ...
    for(i = 0; i < r.versions.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(IbcCoreConnectionV1Version._estimate(r.versions[i]));
    }
    e += 1 + ProtoBufRuntime._sz_enum(IBC/CORE/CONNECTION/V1/CONNECTION_PROTO_GLOBAL_ENUMS.encode_State(r.state));
   ...
   */
  function addVersions(Data memory self, IbcCoreConnectionV1Version.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    IbcCoreConnectionV1Version.Data[] memory tmp = new IbcCoreConnectionV1Version.Data[](self.versions.length + 1);
...
library IbcCoreConnectionV1Counterparty {
  ...
  function _read_prefix(
    ...
    (IbcCoreCommitmentV1MerklePrefix.Data memory x, uint256 sz) = _decode_IbcCoreCommitmentV1MerklePrefix(p, bs);
    r.prefix = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_IbcCoreCommitmentV1MerklePrefix(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcCoreCommitmentV1MerklePrefix.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcCoreCommitmentV1MerklePrefix.Data memory r, ) = IbcCoreCommitmentV1MerklePrefix._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += IbcCoreCommitmentV1MerklePrefix._encode_nested(r.prefix, pointer, bs);
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcCoreCommitmentV1MerklePrefix._estimate(r.prefix));
    return e;
  }
  // empty checker
...
library IbcCoreConnectionV1ClientPaths {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_paths(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_paths(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.paths.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += ProtoBufRuntime._encode_string(r.paths[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
...
library IbcCoreConnectionV1Params {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_uint64(r.max_expected_time_per_block, pointer, bs);
    }
    return pointer - offset;
  }
  // nested encoder
...
library IBC/CORE/CONNECTION/V1/CONNECTION_PROTO_GLOBAL_ENUMS {
