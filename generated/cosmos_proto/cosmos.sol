...
library Cosmos_protoScalarDescriptor {
  ...
  struct Data {
    string name;
    string description;
    COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType[] field_type;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3) {
        if (wireType == ProtoBufRuntime.WireType.LengthDelim) {
          ...
          pointer += _read_unpacked_repeated_field_type(pointer, bs, nil(), counters);
        }
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
    ...
    if (counters[3] > 0) {
      ...
      r.field_type = new COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType[](counters[3]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3 && wireType != ProtoBufRuntime.WireType.LengthDelim) {
        pointer += _read_unpacked_repeated_field_type(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
  ...
  function _read_unpacked_repeated_field_type(
    ...
    COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType x = COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.decode_ScalarType(tmp);
    if (isNil(r)) {
      counters[3] += 1;
    } else {
  ...
  function _read_packed_repeated_field_type(
    ...
    r.field_type = new COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType[](count);
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_varint(
      COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.estimate_packed_repeated_ScalarType(r.field_type),
      pointer,
      bs
    );
    for(i = 0; i < r.field_type.length; i++) {
      int32 _enum_field_type = COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.encode_ScalarType(r.field_type[i]);
      pointer += ProtoBufRuntime._encode_enum(_enum_field_type, pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.name).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.description).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.estimate_packed_repeated_ScalarType(r.field_type));
    return e;
  }
  // empty checker
   ...
   */
  function addFieldType(Data memory self, COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType  value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType[] memory tmp = new COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS.ScalarType[](self.field_type.length + 1);
...
library COSMOS_PROTO_COSMOS_PROTO_GLOBAL_ENUMS {
