...
library CosmosFeegrantV1beta1QueryAllowanceRequest {
...
library CosmosFeegrantV1beta1QueryAllowanceResponse {


  //struct definition
  struct Data {
    CosmosFeegrantV1beta1Grant.Data allowance;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_allowance(pointer, bs, r);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_allowance(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (CosmosFeegrantV1beta1Grant.Data memory x, uint256 sz) = _decode_CosmosFeegrantV1beta1Grant(p, bs);
    r.allowance = x;
    return sz;
  }
   ...
   */
  function _decode_CosmosFeegrantV1beta1Grant(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosFeegrantV1beta1Grant.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosFeegrantV1beta1Grant.Data memory r, ) = CosmosFeegrantV1beta1Grant._decode(pointer, bs, sz);
    ...
    returns (uint)
  {
    ...
    pointer += CosmosFeegrantV1beta1Grant._encode_nested(r.allowance, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosFeegrantV1beta1Grant._estimate(r.allowance));
    return e;
  }
  // empty checker
   ...
   */
  function store(Data memory input, Data storage output) internal {
    CosmosFeegrantV1beta1Grant.store(input.allowance, output.allowance);
...
library CosmosFeegrantV1beta1QueryAllowancesRequest {
...
library CosmosFeegrantV1beta1QueryAllowancesResponse {


  //struct definition
  struct Data {
    CosmosFeegrantV1beta1Grant.Data[] allowances;
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_allowances(pointer, bs, nil(), counters);
      } else
      if (fieldId == 2) {
        pointer += _read_pagination(pointer, bs, r);
      } else
    ...
    pointer = offset;
    if (counters[1] > 0) {
      require(r.allowances.length == 0);
      r.allowances = new CosmosFeegrantV1beta1Grant.Data[](counters[1]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_allowances(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_unpacked_repeated_allowances(
    uint256 p,
    bytes memory bs,
    Data memory r,
    uint[3] memory counters
    ...
    if (isNil(r)) {
      ...
      r.allowances[r.allowances.length - counters[1]] = x;
      counters[1] -= 1;
    }
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.allowances.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        1,
        ProtoBufRuntime.WireType.LengthDelim,
        pointer,
        bs)
      ;
      pointer += CosmosFeegrantV1beta1Grant._encode_nested(r.allowances[i], pointer, bs);
    }
    }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.allowances.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosFeegrantV1beta1Grant._estimate(r.allowances[i]));
    }
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    for(uint256 i1 = 0; i1 < input.allowances.length; i1++) {
      output.allowances.push(input.allowances[i1]);
    }
  ...
  //array helpers for Allowances
  /**
   * @dev Add value to an array
   * @param self The in-memory struct
   * @param value The value to add
   */
  function addAllowances(Data memory self, CosmosFeegrantV1beta1Grant.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosFeegrantV1beta1Grant.Data[] memory tmp = new CosmosFeegrantV1beta1Grant.Data[](self.allowances.length + 1);
    for (uint256 i = 0; i < self.allowances.length; i++) {
      tmp[i] = self.allowances[i];
    }
    tmp[self.allowances.length] = value;
    self.allowances = tmp;
  }
...
library CosmosFeegrantV1beta1QueryAllowancesByGranterRequest {
...
library CosmosFeegrantV1beta1QueryAllowancesByGranterResponse {
