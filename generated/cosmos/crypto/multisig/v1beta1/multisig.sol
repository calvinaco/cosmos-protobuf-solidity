...
library CosmosCryptoMultisigV1beta1MultiSignature {
...
library CosmosCryptoMultisigV1beta1CompactBitArray {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_extra_bits_stored(pointer, bs, r);
      } else
      if (fieldId == 2) {
        pointer += _read_elems(pointer, bs, r);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_uint32(r.extra_bits_stored, pointer, bs);
    }
    if (r.elems.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      2,
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.elems, pointer, bs);
    }
    return pointer - offset;
  }
  // nested encoder
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_uint32(r.extra_bits_stored);
    e += 1 + ProtoBufRuntime._sz_lendelim(r.elems.length);
    return e;
  }
  // empty checker
   ...
   */
  function store(Data memory input, Data storage output) internal {
    output.extra_bits_stored = input.extra_bits_stored;
