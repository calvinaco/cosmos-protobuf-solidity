...
library CosmosEvidenceV1beta1Equivocation {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 4) {
        pointer += _read_consensus_address(pointer, bs, r);
      } else
      {
   ...
   * @return The number of bytes decoded
   */
  function _read_consensus_address(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (string memory x, uint256 sz) = ProtoBufRuntime._decode_string(p, bs);
    r.consensus_address = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    if (bytes(r.consensus_address).length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      4,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_string(r.consensus_address, pointer, bs);
    }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.consensus_address).length);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.consensus_address = input.consensus_address;
