...
library CosmosEvidenceV1beta1MsgSubmitEvidence {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_submitter(pointer, bs, r);
      } else
      if (fieldId == 2) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_string(r.submitter, pointer, bs);
    }
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.submitter).length);
  ...
  function store(Data memory input, Data storage output) internal {
    output.submitter = input.submitter;
...
library CosmosEvidenceV1beta1MsgSubmitEvidenceResponse {
