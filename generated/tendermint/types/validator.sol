...
library TendermintTypesValidatorSet {
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.validators.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += TendermintTypesValidator._encode_nested(r.validators[i], pointer, bs);
    }
    }
    ...
    pointer += TendermintTypesValidator._encode_nested(r.proposer, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.validators.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(TendermintTypesValidator._estimate(r.validators[i]));
    }
    e += 1 + ProtoBufRuntime._sz_lendelim(TendermintTypesValidator._estimate(r.proposer));
   ...
   */
  function addValidators(Data memory self, TendermintTypesValidator.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    TendermintTypesValidator.Data[] memory tmp = new TendermintTypesValidator.Data[](self.validators.length + 1);
...
library TendermintTypesValidator {
  ...
  function _read_pub_key(
    ...
    (TendermintCryptoPublicKey.Data memory x, uint256 sz) = _decode_TendermintCryptoPublicKey(p, bs);
    r.pub_key = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_TendermintCryptoPublicKey(uint256 p, bytes memory bs)
    internal
    pure
    returns (TendermintCryptoPublicKey.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (TendermintCryptoPublicKey.Data memory r, ) = TendermintCryptoPublicKey._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.f_address, pointer, bs);
    }
    ...
    pointer += TendermintCryptoPublicKey._encode_nested(r.pub_key, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(r.f_address.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(TendermintCryptoPublicKey._estimate(r.pub_key));
