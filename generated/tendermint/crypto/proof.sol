...
library TendermintCryptoProof {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 4) {
        pointer += _read_unpacked_repeated_aunts(pointer, bs, nil(), counters);
      } else
      {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 4) {
        pointer += _read_unpacked_repeated_aunts(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.aunts.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += ProtoBufRuntime._encode_bytes(r.aunts[i], pointer, bs);
    }
    }
...
library TendermintCryptoProofOps {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_ops(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
  ...
  function _read_unpacked_repeated_ops(
     ...
     * if `r` is NULL, then only counting the number of fields.
     */
    (TendermintCryptoProofOp.Data memory x, uint256 sz) = _decode_TendermintCryptoProofOp(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
   ...
   * @return The number of bytes used to decode
   */
  function _decode_TendermintCryptoProofOp(uint256 p, bytes memory bs)
    internal
    pure
    returns (TendermintCryptoProofOp.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (TendermintCryptoProofOp.Data memory r, ) = TendermintCryptoProofOp._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.ops.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += TendermintCryptoProofOp._encode_nested(r.ops[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.ops.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(TendermintCryptoProofOp._estimate(r.ops[i]));
    }
    return e;
  }
  // empty checker
   ...
   * @param value The value to add
   */
  function addOps(Data memory self, TendermintCryptoProofOp.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    TendermintCryptoProofOp.Data[] memory tmp = new TendermintCryptoProofOp.Data[](self.ops.length + 1);
