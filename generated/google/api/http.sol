...
library GoogleApiHttp {
  ...
  function _read_unpacked_repeated_rules(
     ...
     */
    (GoogleApiHttpRule.Data memory x, uint256 sz) = _decode_GoogleApiHttpRule(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
   ...
   */
  function _decode_GoogleApiHttpRule(uint256 p, bytes memory bs)
    internal
    pure
    returns (GoogleApiHttpRule.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (GoogleApiHttpRule.Data memory r, ) = GoogleApiHttpRule._decode(pointer, bs, sz);
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.rules.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += GoogleApiHttpRule._encode_nested(r.rules[i], pointer, bs);
    }
    }
    ...
    pointer += ProtoBufRuntime._encode_bool(r.fully_decode_reserved_expansion, pointer, bs);
    }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.rules.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(GoogleApiHttpRule._estimate(r.rules[i]));
    }
    e += 1 + 1;
    return e;
  }
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.fully_decode_reserved_expansion = input.fully_decode_reserved_expansion;
   ...
   */
  function addRules(Data memory self, GoogleApiHttpRule.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    GoogleApiHttpRule.Data[] memory tmp = new GoogleApiHttpRule.Data[](self.rules.length + 1);
...
library GoogleApiHttpRule {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 11) {
        pointer += _read_unpacked_repeated_additional_bindings(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 11) {
        pointer += _read_unpacked_repeated_additional_bindings(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
  ...
  function _read_custom(
    ...
    (GoogleApiCustomHttpPattern.Data memory x, uint256 sz) = _decode_GoogleApiCustomHttpPattern(p, bs);
    r.custom = x;
    return sz;
  }
  ...
  function _read_unpacked_repeated_additional_bindings(
    ...
    if (isNil(r)) {
      ...
      r.additional_bindings[r.additional_bindings.length - counters[11]] = x;
      counters[11] -= 1;
    }
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_GoogleApiCustomHttpPattern(uint256 p, bytes memory bs)
    internal
    pure
    returns (GoogleApiCustomHttpPattern.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (GoogleApiCustomHttpPattern.Data memory r, ) = GoogleApiCustomHttpPattern._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_string(r.delete, pointer, bs);
    }
    ...
    pointer += GoogleApiCustomHttpPattern._encode_nested(r.custom, pointer, bs);
    ...
    pointer += ProtoBufRuntime._encode_string(r.response_body, pointer, bs);
    }
    ...
    for(i = 0; i < r.additional_bindings.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += GoogleApiHttpRule._encode_nested(r.additional_bindings[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.delete).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.patch).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(GoogleApiCustomHttpPattern._estimate(r.custom));
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.body).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.response_body).length);
    for(i = 0; i < r.additional_bindings.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(GoogleApiHttpRule._estimate(r.additional_bindings[i]));
    }
    return e;
  }
  // empty checker
   ...
   * @param value The value to add
   */
  function addAdditionalBindings(Data memory self, GoogleApiHttpRule.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    GoogleApiHttpRule.Data[] memory tmp = new GoogleApiHttpRule.Data[](self.additional_bindings.length + 1);
