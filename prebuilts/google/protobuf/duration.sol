...
import "@protobufcommon/ProtoBufRuntime.sol";
import "@protobufcommon/GoogleProtobufAny.sol";
...
library GoogleProtobufDuration {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_sec(pointer, bs, r);
      } else
      if (fieldId == 2) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_int64(r.sec, pointer, bs);
    }
    if (r.nanos != 0) {
    pointer += ProtoBufRuntime._encode_key(
      2,
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_int64(r.sec);
